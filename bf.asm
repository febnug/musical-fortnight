; bf.asm — Brainfuck interpreter in x86_64 NASM for Linux
; -------------------------------------------------------
; Features
; - Reads program from file path given in argv[1]; if absent, reads from stdin.
...
;
; Build:
;   nasm -felf64 bf.asm -o bf.o && ld -o bf bf.o
; Run:
;   ./bf hello.bf
;   # or: cat hello.bf | ./bf
;
...
section .data
  usage: db "Usage: ./bf <file.bf>  (or pipe program via stdin)", 10
  usage_len: equ $-usage
...
; ---------------------------------------
; _start: read program (file or stdin), filter, preprocess, execute
_start:
  ...
  ; argc is in rdi, argv in rsi when using crt, but with raw _start we have
  ; rdi=argc, rsi=argv, rdx=envp (SysV ABI)
  pop rdi                ; argc
...
.read_from_file:
  ...
  ; Read loop into prog, filtering into-place via r8 as write index
  xor r8, r8             ; write idx
  mov rdi, rbx           ; fd
.read_file_loop:
  mov rsi, prog
  add rsi, r8            ; write position (temp buffer)
  mov rdx, PROGRAM_MAX
  sub rdx, r8
  cmp rdx, 0
  je .done_read_file
  ...
  cmp rax, 0
  jle .done_read_file
  ; Filter rax bytes that just arrived (but we wrote them into prog temporarily)
  mov rcx, rax           ; count
  mov rsi, prog
  add rsi, r8            ; src start of this chunk
...
.stdin_loop:
  ...
  mov r9, r8
  add rdi, r9
  jmp .filter_chunk      ; re-use same filter loop
